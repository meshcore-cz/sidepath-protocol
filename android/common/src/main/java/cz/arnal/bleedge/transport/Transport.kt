 ...
 * `:common` (not in the pure-Kotlin `:bleedge-protocol` SDK) because PHY, the
 * GATT UUIDs, and NODE_INFO framing are Android BLE concerns, not part of the
 * routing protocol.
 */

/** BLEEdge GATT service + characteristic UUIDs (docs/PROTOCOL.md §4.1). */
object BLEEdgeUUIDs {
...
/** Manufacturer company ID tagging BLEEdge adverts; payload = raw 10-byte NodeID. */
const val BLEEDGE_MANUFACTURER_ID = 0xBEED
...
object NodeInfo {
    ...
    fun encode(publicKey: ByteArray, caps: Capabilities): ByteArray {
        require(publicKey.size == BLEEdge.PUBLIC_KEY_BYTES) { "public key must be 32 bytes" }
        val buf = ByteBuffer.allocate(1 + 32 + 2).order(ByteOrder.LITTLE_ENDIAN)
        buf.put(BLEEdge.NODE_INFO_VERSION.toByte())
        buf.put(publicKey)
    ...
    fun decode(data: ByteArray): Decoded? {
        ...
        if ((data[0].toInt() and 0xFF) != BLEEdge.NODE_INFO_VERSION) return null
