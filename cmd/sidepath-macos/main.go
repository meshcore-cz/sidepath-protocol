...
// sidepath-macos is an interactive Sidepath node for macOS (1M PHY).
...
// Use sidepath-listen on Linux for the full Coded PHY PoC.
package main
...
func main() {
	...
	for i, arg := range os.Args[1:] {
		...
		case arg == "--help" || arg == "-h":
			fmt.Fprintf(os.Stderr, `sidepath-macos — interactive Sidepath node for macOS (1M PHY)
...
Bot mode (--bot): the node runs headless and forwards chat messages to a Bun
  ...
  e.g. --channels "Public,rock climbers"). See bots/ for examples:
    sidepath-macos --bot bots/time-bot.ts
    sidepath-macos --bot bots/echo-bot.ts --channels "Public,dev"
...
Bridge mode (--meshcore-bridge): taps a running meshcore-go backend daemon over
  ...
  into the Sidepath mesh as an opaque v3 MESHCORE_PACKET datagram. Phase 1 is
  one-way (MeshCore -> Sidepath). Requires the meshcore-go backend running with
...
NOTE: macOS CoreBluetooth does NOT support LE Coded PHY.
      Use sidepath-listen on Linux for the Long Range PoC.
`)
			os.Exit(0)
		}
	}
	...
	node = blenode.New(blenode.Config{
		...
		OnMessage: func(dg core.Datagram) {
			// Bridge native Sidepath channel messages outward onto the real MeshCore network as a
			// GRP_TXT — exactly once per Sidepath datagram — then ACK_BRIDGED the original sender.
			...
			if meshcoreBridge && dg.Protocol == core.ProtocolSidepathChat {
			...
			// A MESHCORE_PACKET datagram received from another Sidepath node is a raw MeshCore
	...
	if meshcoreBridge {
		...
		go br.Run(ctx, func(pkt mcbridge.Packet) {
			...
			case mcbridge.ForwardDirect:
				...
				if len(cands) == 0 {
					// Dump the known Sidepath nodes' first key byte + name so a hash mismatch
	...
	if botScript != "" {
		...
		theBot = b
		if headless {
			fmt.Fprintf(os.Stderr, "Sidepath macOS bot  node=%s  script=%s  channels=%v\n",
...
func fatalf(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "sidepath-macos: "+format+"\n", args...)
	os.Exit(1)
}
...
// routeLabel describes how the bridge reaches dst over Sidepath: "direct" for a one-hop neighbor,
...
// knownNodeHashes lists every Sidepath node's 1-byte MeshCore hash (public-key[0]) and name, so an
