// sidepath-listen is the Linux CLI for Sidepath using BlueZ D-Bus.
...
import (
	...
	"github.com/meshcore-cz/sidepath-protocol/linux"
)
...
func main() {
	var (
		...
		seedFlag    = flag.String("seed-hex", "", "Ed25519 identity seed (32 bytes / 64 hex chars); loaded from ~/.sidepath/seed if empty. NodeID = pubkey[:8]")
	...
	// Parse identity seed if supplied (else NewNode loads/generates ~/.sidepath/seed)
